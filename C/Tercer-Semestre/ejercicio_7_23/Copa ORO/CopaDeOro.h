...
void mostrarPartidos (Partido *pais)
{
    Partido *aux = NULL;
    for (aux = pais; aux != NULL; aux = aux->siguiente)
    {
        printf("N.%d Partido: ", aux->numPartido); 
        printf("\n%s vs %s ", aux->pais[0].nombre, aux->pais[1].nombre);
        printf("\nFecha del partido: %s", aux->fecha);
        printf("\nHora del partido: %s", aux->hora);
        printf("\nNombre del estadio: %s", aux->estadio);
        printf("\n\n");
    }
}

void mostrarResultados(Nodo *copa)
{
    Nodo *q = NULL;
    Equipo *aux = NULL;
    Registro *datos = NULL; 

 //   printf("\nClub\t\t\tG  E  P  GF  GC  DF  Pts");   
    
    for(q = copa; q != NULL; q = q->siguiente)
    {
        aux = q->pEquipo;
        printf("\nGrupo %c", aux->grupo);
        printf("\nClub\t\t\tG  E  P  GF  GC  DF  Pts");   
        for(aux = q->pEquipo; aux != NULL; aux = aux->siguiente)
        {   
            datos = aux->record; 
            //printf("\nGrupo %c", aux->grupo); 
            printf("\n%s\t\t\t%d  %d  %d  %d  %d  %d  %d", aux->nombre, datos->ganados, datos->empatados, datos->perdidos, datos->golesAfavor, datos->golesEnContra, datos->diferenciaGoles,datos->puntos); 
        }
    }
}